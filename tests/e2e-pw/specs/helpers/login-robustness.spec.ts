...
test.describe( 'Login Helper Robustness', () => {
	...
	test( 'should login successfully from logged out state', async ( {
		page,
	} ) => {
	...
	test( 'should handle already logged in state gracefully', async ( {
		page,
	} ) => {
		// Login first
		await loginAsAdmin( page );
	...
	test( 'should logout successfully from logged in state', async ( {
		page,
	} ) => {
		// Login first
		await loginAsAdmin( page );
	...
	test( 'should handle already logged out state gracefully', async ( {
		page,
	} ) => {
		// Verify we're logged out
		expect( await isLoggedIn( page ) ).toBe( false );
	...
	test( 'should maintain login state across page navigations', async ( {
		page,
	} ) => {
		await loginAsAdmin( page );
		expect( await isLoggedIn( page ) ).toBe( true );
	...
	test( 'should detect login state correctly after manual logout', async ( {
		page,
	} ) => {
		// Login first
		await loginAsAdmin( page );
