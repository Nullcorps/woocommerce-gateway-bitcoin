...
test.describe('Schedule payment checks', () => {
  ...
  async function isJobScheduledForOrder(page: any, orderId: string): Promise<boolean> {
    const tableRow = await getActionSchedulerTableRowForOrder(page, orderId);
  ...
  async function runActionInRow(page: any, actionSchedulerTableRow: any) {
    const hookColumn = actionSchedulerTableRow.locator('td[data-colname="Hook"]');
  ...
  test('should schedule a payment check when a Bitcoin order is placed', async ({ page }) => {
  ...
  test('should schedule a payment check when a Bitcoin orders status is set to on-hold', async ({ page }) => {
    const orderId = await placeBitcoinOrder(page);
  ...
  test('should cancel the scheduled check when the order is marked paid', async ({ page }) => {
    const orderId = await placeBitcoinOrder(page);
  ...
  test('should schedule new payment check after each check that does not have payment', async ({ page }) => {
    const orderId = await placeBitcoinOrder(page);
