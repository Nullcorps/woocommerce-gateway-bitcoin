...
async function verifyLoginSuccess(page: Page): Promise<void> {
  ...
  if (currentUrl.includes('wp-login.php')) {
    ...
    if (!hasSuccessMessage) {
      throw new Error('Login appears to have failed - still on login page with no success message');
    }
  }
  ...
  if (!isInAdmin) {
    ...
    try {
      await page.waitForSelector('#wpadminbar, #adminmenumain', { timeout: 10000 });
    } catch (error) {
      // Check if we got redirected back to login
      const finalUrl = page.url();
      if (finalUrl.includes('wp-login.php')) {
        throw new Error('Navigation to wp-admin redirected to login page - login failed');
      }
      throw error;
    }
  }
  ...
  if (!loginStatus) {
    throw new Error('Login verification failed - login status check returned false');
  }
...
export async function isLoggedIn(page: Page): Promise<boolean> {
  try {
    ...
    try {
      const response = await page.request.get('/wp-admin/admin-ajax.php', {
      ...
      if (response.status() === 200) {
        ...
        if (responseText.includes('"nonces_expired"') || responseText.includes('"logged_in":false')) {
          return false;
        }
 ...
 */
export async function loginAsAdminWithRetry(page: Page, maxRetries: number = 3): Promise<void> {
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      if (attempt === maxRetries) {
        throw new Error(`Failed to login after ${maxRetries} attempts. Last error: ${error}`);
      }
