...
export const Edit: React.FC<EditProps> = ({ attributes, setAttributes, context }) => {
  ...
  console.log('(edit)orderId' + orderId);
	...
	return (
		<>
			<InspectorControls>
				<PanelBody title={__('Payment Status Settings', 'bh-wp-bitcoin-gateway')}>
					<ToggleControl
			...
			<div {...blockProps}>
				<PaymentStatusDisplay showLabel={showLabel} isPreview={true} orderId={effectiveOrderId} />
				<p className="description" style={{ fontSize: '12px', opacity: 0.7, marginTop: '4px' }}>
					{__('Preview - actual status will display on order confirmation pages', 'bh-wp-bitcoin-gateway')}
				</p>
			</div>
		</>
	);
