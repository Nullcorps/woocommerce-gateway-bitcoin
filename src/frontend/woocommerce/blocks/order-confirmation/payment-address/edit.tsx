...
export const Edit: React.FC<EditProps> = ({ attributes, setAttributes, context }) => {
  ...
  console.log('(edit)orderId' + orderId);
	...
	return (
		<>
			<InspectorControls>
				<PanelBody title={__('Payment Address Settings', 'bh-wp-bitcoin-gateway')}>
					<ToggleControl
			...
			<div {...blockProps}>
				<PaymentAddressDisplay showLabel={showLabel} isPreview={true} orderId={effectiveOrderId} />
				<p className="description" style={{ fontSize: '12px', opacity: 0.7, marginTop: '4px' }}>
					{__('Preview - actual address will display on order confirmation pages', 'bh-wp-bitcoin-gateway')}
				</p>
			</div>
		</>
	);
